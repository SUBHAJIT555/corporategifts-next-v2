...
const FAQAccordionItem = memo(function FAQAccordionItem({
  ...
  faq: (typeof productFaqData)[number];
  isOpen: boolean;
  isLast: boolean;
  onToggle: () => void;
}) {
      ...
      <RevealSection className="mx-auto max-w-7xl border-x border-hairline px-5 py-3 sm:px-6 sm:py-4 lg:py-6">
        ...
        >
          ...
          <p className="mt-4 text-body-md text-muted sm:text-[17px] sm:leading-7">
            Find quick answers about our product categories, customization
            options, and ordering process.
          </p>
        </Reveal>

        <Reveal animationNum={1} className="mx-auto max-w-4xl">
          <div>
            {productFaqData.map((faq, index) => (
              <FAQAccordionItem
                key={faq.id}
                faq={faq}
                isOpen={openIndex === index}
                isLast={index === productFaqData.length - 1}
